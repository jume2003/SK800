...
{
    class GelWarehouseViewModel:LogicStepScreen
    {
        ...
        public IList<VBJ> BjList
        {
            get
            {
                if (_BjList == null)
                {
                    String key = typeof(T_BJ_GelWarehouse).Name;
                    if (Constants.BJDict.ContainsKey(key))
                    {
                        _BjList = Constants.BJDict[key].ToList();
                    }
                    else
                    {
        ...
        public IList<KeyValuePair<Byte, String>> ActionList
        {
            get
            {
                if (_ActionList == null)
                {
                    ...
                    _ActionList.Add(new KeyValuePair<byte, string>(Convert.ToByte(0), "移动"));
                }
                return _ActionList;
            }
        }
