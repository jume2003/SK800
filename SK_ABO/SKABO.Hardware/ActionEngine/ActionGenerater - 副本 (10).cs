    ...
    public class ActionGenerater
    {
        ...
        public Sequence GenerateInjectActGroup(ActionPoint[] tagers,bool is_asc=true)
        {
            ...
            foreach (var group in tager_group)
            {
                ...
                GenerateInjectActGroup(ref move_y, hit_sort,ref index, ref tager, ref point, ref width, ref is_ok, move_x);
                if (is_ok)
                {
        ...
        //生成加样移动组合
        public void GenerateInjectActGroup(ref Sequence seque,int []hitsort,ref int index, ref ActionPoint[] tager, ref int[] point, ref int[] width, ref bool is_ok, ActionBase move_x=null)
        {
            int hit_index = 0;
            ...
            else
            {
                ...
                if(done_points.Count!=0)
                {
                    ...
                    else if (done_points[0].type == TestStepEnum.PutTip)
                    {
                        ...
                        sequ.AddAction(InjectMoveTo.create(3001, entcloses.ToArray(), -1, IMask.Gen(-1), injz, 100));
                        for (int i = 0; i < done_points.Count; i++)
                        {
                            injz[entcloses[i].Index] = done_points[i].zb;
                        }
                    ...
                    else if (done_points[0].type == TestStepEnum.AbsLiquid)
                    {
                        ...
                        for (int i = 0; i < done_points.Count; i++)
                        {
                            ...
                            absorbs[hit_index] = -(done_points[i].capacity);
                        }
                    ...
                    else if (done_points[0].type == TestStepEnum.MixLiquid)
                    {
                        ...
                        for (int i = 0; i < done_points.Count; i++)
                        {
                            ...
                            {
                                injz[entcloses[i].Index] = 0;
                                mix_seq_act.AddAction(InjectMoveTo.create(3001, ent_tem.ToArray(), -1, IMask.Gen(-1), injz));
                                absorbs[entcloses[i].Index] = -(done_points[i].backcapacity);
                                mix_seq_act.AddAction(InjectAbsorb.create(3001, ent_tem.ToArray(), done_points[i].backspeed, absorbs));
                            }
                            injz[entcloses[i].Index] = done_points[i].zb;
                            mix_seq_act.AddAction(InjectMoveTo.create(3001, ent_tem.ToArray(), -1, IMask.Gen(-1), injz));
