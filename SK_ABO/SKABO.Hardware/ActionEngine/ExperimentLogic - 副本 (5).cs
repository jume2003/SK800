...
namespace SKABO.ActionEngine
{
    public class ExperimentLogic
    {
        ...
        public List<LogicActionGoup> inj_action_list = new List<LogicActionGoup>();
        ...
        public void ClsAction()
        {
            ...
            foreach (var package in experiment_package_list)
            {
                ...
                {
                    if (package.action_list[i].State == 2)
                    {
        ...
        public void UpDataAction()
        {
            lock (mylock)
            {
                ...
                var action_list = new List<T_GelStep>();
                ...
                if (pier_y != 0 && action_list.Count != 0 && action_list[0].StepClass != TestStepEnum.KaiKongGel)
                {
                    var pier_back = MoveTo.create(piercerDevice, 3000, -1, 0, -1);
                    pier_back.runAction();
                }
                //创建组合
                CreateActionGroup(action_list, inj_action_list);
                foreach(var group_tem in inj_action_list)
                {
                ...
                //查找连续试剂(如果超过三个就合并)
                //交叉配血合并
                //AgentiaCombination(action_list, 0);
                //CrossMatchingCombination(action_list, 0);
                //SlysCombination(action_list, 0);


            }
        }
        //创建组合
        public void CreateActionGroup(List<T_GelStep> action_list, List<LogicActionGoup> group_list)
        {
        ...
        public void AgentiaCombination(List<T_GelStep> action_list,int index)
        {
            ...
            string liquidtype = "";
            int gel_test_id = -1;
            while(index < action_list.Count)
            {
                var act = action_list[index];
            ...
            if(agentia_list.Count>2)
            {
                ...
                {
                    agentia_list[i].is_skip_puttip = i< (agentia_list.Count-1);
        ...
        public void CrossMatchingCombination(List<T_GelStep> action_list, int index)
        {
            ...
            string samplecode = "";
            int gel_test_id = -1;
            bool is_hxborxueqing = false;
            bool is_hxbslys = false;
            while (index < action_list.Count)
            {
                var act = action_list[index];
                var exp_tem = (ExperimentPackage)act.ExperPackage;
                var liquidinfo = act.GetLiquidInfo();
                var fpytinfo = act.GetFpytInfo();
                is_hxborxueqing = liquidinfo == null ? false :liquidinfo.LiquidType.IndexOf("病人") != -1;
                is_hxbslys = fpytinfo == null|| liquidinfo==null ? false : liquidinfo.IsSlys&&fpytinfo.FindLiquidType("病人*");
                if (liquidinfo != null && exp_tem.is_crossmatching && (is_hxborxueqing|| is_hxbslys )&& (exp_tem.GetSampleCode(act.SampleIndex)== samplecode|| samplecode=="") && (liquidinfo.LiquidType == liquidtype || liquidtype == "") && (exp_tem.gel_test_id == gel_test_id || gel_test_id == -1))
                {
                    liquidtype = act.GetLiquidInfo().LiquidType;
                    samplecode = exp_tem.GetSampleCode(act.SampleIndex);
                    gel_test_id = exp_tem.gel_test_id;
                    agentia_list.Add(act);
                    if (agentia_list.Count == 12)
                    {
            ...
            if (agentia_list.Count > 1)
            {
                ...
                is_hxbslys = agentia_list[0].GetLiquidInfo().IsSlys;
                agentia_list[0].skip_spu_times = (agentia_list[0].is_mix|| is_hxbslys)? 1:agentia_list.Count;
                agentia_list[0].after_mix_spucapacity = agentia_list[0].is_mix ? agentia_list.Count:1;
            }
            if (index < inj_action_list.Count)
                CrossMatchingCombination(action_list, index);
        }
        //是否在离心机内的所有卡当前动作是离心
        ...
        public bool CenRunLogic(CentrifugeMDevice cendev)
        {
            ...
            if (is_allcenact && is_allputok && !is_empty && (!is_gelsoon || is_gelfull))
            {
                ...
                if (resinfo_list.Count() % 2 != 0)
                {
                    ...
                    var exp_pack = ExperimentPackage.Create(pei_step.ToList(), pei_gel.GetCodeAt(0), "123",8, 0,1,0,false,0,false);
        ...
        public void ActLogic(List<T_GelStep> act_group, List<T_GelStep> act_group_next,double dt)
        {
            if (act_group!=null&&act_group.Count != 0 && act_group[0].State == 0)
            {
                ...
                if (act_group[0].InjectCount != 0)
                {
                    ...
                    act_group.OrderByDescending(c => ((ExperimentPackage)c.ExperPackage).piercer_index);
                        ...
                        case TestStepEnum.XJPD:
                            {
                                ...
                                if (put_seat != null && wasted_seat != null && !(resinfo.Purpose == "4" && actionmanager.getAllActionsCount(piercerDevice) != 0))
                                {
                                    ...
                                    seque_act.AddAction(SkCallBackFun.create((ActionBase acttem) =>
                                    {
                                        ...
                                        {
                                            exper_package.samples_barcode[i] = exper_package.samples_barcode[i]+"_used" + Engine.getInstance().getSystemMs();
                                        }
                                        for (int i = 0; i < exper_package.volunteers_barcode.Count; i++)
                                        {
                                            exper_package.volunteers_barcode[i] = exper_package.volunteers_barcode[i] + "_used" + Engine.getInstance().getSystemMs();
                                        }
                                        return true;
                                    }));
                                    //是否还有动作
        ...
        public int GetInjectCount()
        {
            ...
            //    if (i == 3&&exp_pack_cur.is_crossmatching==false&&inj_action_list_tem[i].StepClass == TestStepEnum.FPYT&&exp_pack_cur.samples_barcode_index[inj_action_list_tem[i].SampleIndex]==0)
            //    {
        ...
        public void Logic(double dt)
        {
            ...
            //        var fpytinfo = action_tem.GetFpytInfo();
            ...
            //            inject_count = 2;
            //        }
            //        //如果是交叉配血(病人红细胞与血清用一个管)
            //        if (exp_pack.is_crossmatching && fpytinfo != null && fpytinfo.FindLiquidType("病人*"))
            //        {
            //            var sample_list_tem =  exp_pack.samples_barcode.Where((x, i) => exp_pack.samples_barcode.FindIndex(z => z == x) == i);
            //            inject_count = sample_list_tem.Where(item => item.IndexOf("used") == -1).Count();
            //        }
