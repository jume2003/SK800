...
namespace SK_ABO.ActionEngine
{
    ...
    public class MoveTo : Action
    {
        ...
        public static MoveTo create(double ttime = 0, int xx = -1, int yy = -1, int zz = -1)
        {
        ...
        public override void run(double dt)
        {
            ...
            switch(step)
            {
                case 0:
                    ...
                    if(z >= 0) resultz = device.MoveZ((int)z);
    ...
    public class HandTakeCard : Action
    {
        ...
        public bool zdone { get; set; } = false;
        public bool zldone { get; set; } = false;
        public bool zcdone { get; set; } = false;
        public bool zbone { get; set; } = false;
        ...
        public override void run(double dt)
        {
            ...
            switch (step)
            {
                ...
                case 1:
                    ...
                    if (resultx&&resulty) step++;
                    break;
                case 2:
                    if (xdone == false) xdone = device.DoneX();
                    if (ydone == false) ydone = device.DoneY();
                    if(xdone && ydone) step=7;
                    break;
                case 3:
                    int[] z_tem = { -1, -1, -1, -1 };
                ...
                case 4:
                    for (int i = head_index; i < tipdata.count; i++)
                    {
                        if (zdone[i] == false) xdone = device.DoneInjectZ(i);
                    }
